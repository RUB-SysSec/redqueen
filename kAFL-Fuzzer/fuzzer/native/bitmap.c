 ...
 */
uint64_t are_new_bits_present(uint8_t* bitmap, uint8_t* new_bitmap, uint64_t bitmap_size) {
  uint64_t bit_count = 0;
  uint64_t byte_count = 0;
  //bool new_bits = false;

  for (uint64_t i = 0; i < bitmap_size; i++) {
    //Check if a bit is set in the new map
    if (new_bitmap[i] != 0xff && bitmap[i] != 0xff) {
      //Assign bucket to value
      new_bitmap[i] = bucket_lut[new_bitmap[i]+1];
      //Is the bucket empty?
      if (((new_bitmap[i]) & bitmap[i]) == 0)  {
        //new_bits = true;
        //Set bit => Bucket is occupied
        if (!bitmap[i]){
          byte_count++;
        }
        bit_count++;
...
bool is_finding_unique(uint8_t* bitmap, uint8_t* new_bitmap, uint64_t bitmap_size) {
  bool new_bits = false;

  for (uint64_t i = 0; i < bitmap_size; i++) {
    //Check if a bit is set in the new map
    if (new_bitmap[i] != 0xff && bitmap[i] == 0x00) {
      bitmap[i] = 0xff;
      new_bits = true;
    }
  }
