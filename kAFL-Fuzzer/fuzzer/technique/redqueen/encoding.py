...
class SextEncoding(Encoding):
    ...
    def is_applicable(self, cmp, lhs, rhs):
        ...
        return SextEncoding._is_applicable_sext(cmp, self.bytes, lhs) and SextEncoding._is_applicable_sext(cmp, self.bytes, rhs) 
...
class ZextEncoding(Encoding):
    ...
    def is_applicable(self, cmp, lhs, rhs):
        ...
        lhs = self.apply_reverse(lhs)
        rhs = self.apply_reverse(rhs)
        return ZextEncoding._is_applicable_zext(cmp, self.bytes, lhs) and ZextEncoding._is_applicable_zext(cmp, self.bytes, rhs)
...
Encoders =  [ ZextEncoding(bytes, reverse) for (bytes,reverse) in product([1,2,4], [True, False]) ] +\
            ...
            [ CStringEncoding()]+\
            [ MemEncoding(length) for length in xrange(4,16)]+\
