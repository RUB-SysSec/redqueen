...
class InfoConfiguration:
    ...
    def __load_arguments(self):
        ...
        parser_vm = subparsers.add_parser('VM', help="Full Virtual Machine Mode (RAM File / Overlay Files)", formatter_class=argparse.RawTextHelpFormatter)
        ...
        parser_kernel = subparsers.add_parser('Kernel', help="Lightweight Linux Mode (Kernel Image / initramfs)", formatter_class=argparse.RawTextHelpFormatter)
        ...
        for sparser in [parser_vm, parser_kernel]:
            sparser.add_argument('executable', metavar='<Info Executable>', action=FullPath, type=parse_is_file, help='path to the info executable (kernel address dumper).')
            sparser.add_argument('mem', metavar='<RAM Size>', help='size of virtual RAM (default: 300).', default=300, type=int)
            sparser.add_argument('-v', required=False, help='enable verbose mode (./debug.log).', action='store_true', default=False)
            sparser.add_argument('-macOS', required=False, help='enable macOS Support (requires Apple OSK)', action='store_true', default=False)
...
class DebugConfiguration:
    ...
    def __load_arguments(self):
        ...
        debug_modes = ["benchmark", "trace", "trace-qemu", "noise", "noise-multiple", "printk", "redqueen", "redqueen-qemu",
                       "cov","verify"]
        ...
        for sparser in [parser_vm, parser_kernel]:
            ...
            sparser.add_argument('payload', metavar='<Payload Data>', action=MapFullPaths, type=parse_is_file, nargs="+", help='path to the payload file.')

            sparser.add_argument('debug_mode', metavar='<Debug Mode>', choices=debug_modes, help=debug_modes_help)

            sparser.add_argument('-R', required=False, help='disable fast reload mode', action='store_false', default=True)

            sparser.add_argument('-ip0', required=False, metavar='<IP-Filter 0>', type=parse_range_ip_filter, help='instruction pointer filter range 0')
            sparser.add_argument('-ip1', required=False, metavar='<IP-Filter 1>', type=parse_range_ip_filter, help='instruction pointer filter range 1 (not supported in this version)')
            sparser.add_argument('-ip2', required=False, metavar='<IP-Filter 2>', type=parse_range_ip_filter, help='instruction pointer filter range 2 (not supported in this version)')
            sparser.add_argument('-ip3', required=False, metavar='<IP-Filter 3>', type=parse_range_ip_filter, help='instruction pointer filter range 3 (not supported in this version)')

            sparser.add_argument('-i', metavar='<Iterations>', help='debug iterations (default: 5)', default=5, type=int)
            sparser.add_argument('-V', required=False, help='lazy vAPIC resets in VM reload mode (results in performance boost, but may increase non determinism)', action='store_true', default=False)
...
class FuzzerConfiguration:
    ...
    def __load_arguments(self):
        ...
        for sparser in [parser_vm, parser_kernel]:
            ...
            sparser.add_argument('-ip0', required=True, metavar='<IP-Filter 0>', type=parse_range_ip_filter, help='instruction pointer filter range 0')
            sparser.add_argument('-ip1', required=False, metavar='<IP-Filter 1>', type=parse_range_ip_filter, help='instruction pointer filter range 1 (not supported in this version)')
            sparser.add_argument('-ip2', required=False, metavar='<IP-Filter 2>', type=parse_range_ip_filter, help='instruction pointer filter range 2 (not supported in this version)')
            sparser.add_argument('-ip3', required=False, metavar='<IP-Filter 3>', type=parse_range_ip_filter, help='instruction pointer filter range 3 (not supported in this version)')


            sparser.add_argument('-p', required=False, metavar='<Process Number>', help='number of worker processes to start.', default=1, type=int)
            sparser.add_argument('-t', required=False, metavar='<Task Number>', help='tasks per worker request to provide.', default=1, type=int)
            sparser.add_argument('-v', required=False, help='enable verbose mode (./debug.log).', action='store_true',
                                default=False)
            sparser.add_argument('-g', required=False, help='disable GraphViz drawing.', action='store_false', default=True)
            ...
            sparser.add_argument('-i', required=False, type=parse_ignore_range, metavar="[0-131072]",
                                ...
                                action='append')
            sparser.add_argument('-e', required=False, help='disable evaluation mode.', action='store_false', default=True)
            sparser.add_argument('-D', required=False, help='skip deterministic stages (dumb mode).',action='store_false', default=True)
            sparser.add_argument('-I', required=False, metavar='<Dict-File>', help='import dictionary to fuzz.', default=None, type=parse_is_file)
            sparser.add_argument('-macOS', required=False, help='enable macOS Support (requires Apple OSK)', action='store_true', default=False)
            sparser.add_argument('-f', required=False, help='disable fancy UI', action='store_false', default=True)
            sparser.add_argument('-r', required=False, help='enable fast redqueen insertion algorithm', action='store_true', default=False)
            sparser.add_argument('-n', required=False, help='disable filter sampling', action='store_false', default=True)
            sparser.add_argument('-l', required=False, help='enable UI log output', action='store_true', default=False)

            sparser.add_argument('-fix_hashes', required=False, help='enable checksum fix module', action='store_true', default=False)
            sparser.add_argument('-hammer_jmp_tables', required=False, help='enable jump table hammering', action='store_true', default=False)
