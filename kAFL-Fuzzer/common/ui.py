...
class FuzzerUI():
    ...
    def __init__(self, process_num, fancy=True, inline_log=True, redqueen=False):
        if not fancy:
            self.HLINE = '-'
            self.VLINE = '|'
            self.VLLINE = '+'
        ...
        self.target_name = os.path.basename(FuzzerConfiguration().argument_values['executable'])
    ...
    def __loading_screen(self):
        print(self.CLRSCR + self.__get_logo(loading_screen=True) +
              " " + self.VLINE + " Loading QEMU processes...                                        " + self.VLINE).encode('utf-8')
    ...
    def __get_logo(self, loading_screen=False):
        ...
        if not self.state or loading_screen:
            ...
            imports = self.VLINE + " Imports:         " + self.__get_printable_integer(self.state["imports"])  + " " + self.VLINE
            num = self.state["preliminary"]
            if num != 0:
                self.print_findings = True
                new_findings = " New Findings:      " + self.__get_printable_integer(num, color=(self.WARNING + self.BOLD)) + " "
            else:
                new_findings = " New Findings:      " + self.__get_printable_integer(num, color=(self.BOLD)) + " "
    ...
    def __get_ui_line3(self):
        if self.redqueen:
            if self.state["progress_requeen_amount"] == 0:
                ...
                try:
                    requeen_rate = float(self.state["progress_redqueen"]) / float(self.state["progress_requeen_amount"])
                except:
                    requeen_rate = 0.0
            ...
            bits = str(self.__get_printable_float(self.state["ratio_bits"])).replace("%", "b")
    ...
    def __get_ui_line13(self):
        return " " + self.VRLINE + (25 * self.HLINE) + self.HR + (23 * self.HLINE) + self.HR + (16 * self.HLINE) + self.VLLINE + "\n"
