...
def check_if_nativ_lib_compiled():
    if not (os.path.exists("./fuzzer/native/") and os.path.exists("./fuzzer/native/bitmap.so")):
        print(WARNING + WARNING_PREFIX + "bitmap.so file does not exist. Compiling..." + ENDC)
        p = subprocess.Popen(("gcc fuzzer/native/bitmap.c --shared -fPIC -O3 -o fuzzer/native/bitmap.so").split(" "),  stdout=subprocess.PIPE,  stdin=subprocess.PIPE,  stderr=subprocess.PIPE)
        if p.wait() != 0:
...
def check_if_installed(cmd):
    p = subprocess.Popen(("which " + cmd).split(" "),  stdout=subprocess.PIPE,  stdin=subprocess.PIPE,  stderr=subprocess.PIPE)
    if p.wait() != 0:
        return False
    return True
...
def check_kafl_ini():
    if not os.path.exists("./kafl.ini"):
