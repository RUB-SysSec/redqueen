...
def prepare_working_dir(directory_path, purge=False):
    folders = ["/findings/", "/corpus", "/findings/crash",  "/findings/non_uniq", "/findings/kasan", "/findings/timeout", "/rbuf", "/evaluation", "/bitmaps", "/preliminary", "/yield", "/yield/preliminary", "/yield/corpus", "/imports"]
    ...
    for (directory, _, files) in os.walk(seed_directory):
        for f in files:
            ...
            if os.path.exists(path):
                copyfile(path, working_directory + "/corpus/" + "seed_%05d"%i)
                i += 1
